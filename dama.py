...
print(ops_util.resource_path('dc64.dll'))
dll = ctypes.windll.LoadLibrary(ops_util.resource_path('dc64.dll'))
...
if __name__ == '__main__':
    client = dcVerCode('coolplay007', 'zzqweq1w2e3', '0');  # 超人打码帐号,超人打码密码,软件ID
    img = open('image.png', 'rb')
    buffer = img.read()
    img.close()

    img2 = pyautogui.screenshot(region=(735,415,136,55))
    # img2 = pyautogui.screenshot()
    img2.save("yzm.png")

    buffer2 = img2.tobytes()

    # 查询帐号余额
    print('帐号余额:' + client.getUserInfo())

    # 按图片字节数据识别
    # yzm, imageId = client.recByte(buffer2)
    # print('识别结果：' + yzm, '验证码ID：' + imageId)

    # 按图片本地路径识别
    yzm, imageId = client.recYZM("yzm.png")
    print('识别结果：' + yzm, '验证码ID：' + imageId)
