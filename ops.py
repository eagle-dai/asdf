...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.hi_there["command"] = self.gua_ji
        self.hi_there.grid(row=3)


        self.start_in_char= tk.Button(self)
        self.start_in_char["text"] = "开始"
        self.start_in_char["command"] = self.kai_shi
        self.start_in_char.grid(row=3, column=1)

        self.start_in_char= tk.Button(self)
        self.start_in_char["text"] = "战斗"
        self.start_in_char["command"] = self.zhan_dou
        self.start_in_char.grid(row=3, column=2)

        self.start_in_char = tk.Button(self)
        self.start_in_char["text"] = "继续"
        self.start_in_char["command"] = self.zhan_dou
        self.start_in_char.grid(row=3, column=3)

        self.hi_there = tk.Button(self)
        self.hi_there["text"] = "停止"
        self.hi_there["command"] = self.stop
        self.hi_there.grid(row=4)
    ...
    def stop(self):
        logging.info("stop!")
        gcf.Gcfg.running = False

    def kai_shi(self):
        logging.info("kai shi!")
        gcf.Gcfg.running = True
        _thread.start_new_thread(game.start_kaishi, ())
